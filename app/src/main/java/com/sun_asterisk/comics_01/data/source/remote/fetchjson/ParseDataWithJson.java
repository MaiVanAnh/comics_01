...
public class ParseDataWithJson {
    ...
    public List<Comic> parseJsonStringOriginToDataObjectList(String json) {
        try {
            JSONObject jsonObject = new JSONObject(json);
            JSONArray jsonArray = jsonObject.getJSONArray(Comic.ComicEntry.COMIC_LIST);
            return parseJsonArrayToDataObjectList(jsonArray);
    ...
    public List<Comic> parseJsonArrayToDataObjectList(JSONArray jsonArrayComicList) {
        List<Comic> comicList = new ArrayList<>();

        try {
            for (int index = 0; index < jsonArrayComicList.length(); index++) {
                JSONObject comicJson = jsonArrayComicList.getJSONObject(index);
                Comic comic = parseJsonObjectToDataObject(comicJson);
                comicList.add(comic);
            }
        } catch (JSONException e) {
            e.printStackTrace();
    ...
    private Comic parseJsonObjectToDataObject(JSONObject jsonObjectComic) {
